...
/**
 * Created by Faust on 05.10.2016.
 */
public class XmlTask
{
        ...
        private class Registration  {
            ...
            public double gasReg = 0;
                ...
                private final String path = "C://Users/Faust/Desktop/Java/starting-monkey-to-human-path/src/PO42y/Pertsev/wdad/learn/xml/housekeeper.xml";
    ...
    public void setTariff(String tariffName, double newValue) throws IOException {
            ...
            updateDocument();
                ...
                public void addRegistration (String street, int buildingNumber, int flatNumber,
                    ...
                    NodeList buildings = doc.getElementsByTagName("building");
                    int sss = buildings.getLength();

                    NodeList flats = getFlatsNeededBuild(buildings, street, buildingNumber);

                    for (int i = 0; i < flats.getLength(); i++) {
                        int str = flats.getLength();//.getAttributes().getNamedItem("number");//.getNodeValue();
    ...
    private NodeList getFlatsNeededBuild(NodeList buildings, String street, int buildingNumber){
        ...
        if(buildings != null){
                for (int i = 0; i < buildings.getLength(); i++) { //поиск нужного дома
                    String _street = buildings.item(i).getAttributes().getNamedItem("street").getNodeValue();
                    int _buildigNumber = Integer.valueOf(buildings.item(i).getAttributes().getNamedItem("number").getNodeValue());

                        if(_street.equals(street) && _buildigNumber == buildingNumber){
